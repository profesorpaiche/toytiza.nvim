local util = {}
local chalkboard = require("chalkboard.theme")
...
function util.onColorScheme()
    if vim.g.colors_name ~= "chalkboard" then
        vim.cmd([[autocmd! chalkboard]])
        vim.cmd([[augroup! chalkboard]])
    end
end
...
util.contrast = function()
    vim.cmd([[augroup chalkboard]])
    vim.cmd([[autocmd!]])
    vim.cmd([[autocmd ColorScheme * lua require("chalkboard.util").onColorScheme()]])
...
function util.load()
    ...
    local editor = chalkboard.loadEditor()
    local syntax = chalkboard.loadSyntax()
    local treesitter = chalkboard.loadTreeSitter()
    local filetypes = chalkboard.loadFiletypes()
    ...
    chalkboard.loadTerminal()

    -- imort tables for plugins and lsp
    local plugins = chalkboard.loadPlugins()
    local lsp = chalkboard.loadLSP()
